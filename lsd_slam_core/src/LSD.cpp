        ...
        else
        {
            ...
            if(imageDist.rows != h_inp || imageDist.cols != w_inp)
            {
                if(imageDist.rows * imageDist.cols == 0)
                    log::error("failed to load image %s! skipping.\n", files[i].c_str());
                else
                    log::error("image %s has wrong dimensions - expecting %d x %d, found %d x %d. Skipping.\n",
                            files[i].c_str(),
...
int main( int argc, char** argv )
{
	...
	if(undistorter == 0)
	{
        log::fatal("need camera calibration file! (set using -c FILE)\n");
		exit(1);
	}
	...
	{
        log::fatal("need source files! (set using -f FOLDER or KLG)\n");
		exit(1);
	}
    ...
    else
    {
        ...
        {
            log::info("found %d image files in folder %s!\n", (int)files.size(), source.c_str());
        }
        else if(getFile(source, files) >= 0)
        {
            log::info("found %d image files in file %s!\n", (int)files.size(), source.c_str());
        }
        else
        {
            log::error("could not load file list! wrong path / file?\n");
        }
